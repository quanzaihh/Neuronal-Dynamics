...
1、考虑突触输入电流为$\frac{q}{\tau_s}e^{-\frac{t-t_f}{\tau_s}}$($t>t_f$)，$t_f$是电流到达突触的时间。\\
\textbf{(a)求膜电压响应}\\\\
代入公式\ref*{卷积}，求解其卷积响应即可。我的信号与系统有点忘光了，求了蛮久，响应如下(没化简)：

\begin{equation}
    u(t)=u_{reset}+\frac{qR}{\tau_m-\tau_s}e^{-\frac{t-t_f}{\tau_s}}[e^{\frac{\tau_m-\tau_s}{\tau_m\tau_s}t}-e^{\frac{\tau_m-\tau_s}{\tau_m\tau_s}t_f}]
    \label{1.a的解}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        &\lim_{\tau_s\to\tau_m}=\frac{e^{\frac{\tau_m-\tau_s}{\tau_m\tau_s}t}-e^{\frac{\tau_m-\tau_s}{\tau_m\tau_s}t_f}}{\tau_s\-\tau_m}\\
        &\text{令$x=\frac{\tau_m-\tau_s}{\tau_m\tau_s}$}\\
        &=\lim_{x\to0}=\frac{e^{xt}-e^{xt_f}}{\tau_m\tau_sx}\\
        &\text{洛必达}\\
        &=\lim_{x\to0}=\frac{te^{xt}-t_fe^{xt_f}}{\tau_m\tau_s}\\
